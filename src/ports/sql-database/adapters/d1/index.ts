...
export class D1Database extends SqlDatabase {
  constructor(private readonly database: D1DatabaseBinding, private readonly telemetry?: Telemetry) {
    super('d1');
  }
  ...
  async batch(statements: readonly D1BatchStatement[]): Promise<readonly unknown[]> {
    return observe(this.telemetry, 'sql', this.adapter, 'batch', () => this.database.batch(statements.map(({ sql, parameters = [] }) => (
